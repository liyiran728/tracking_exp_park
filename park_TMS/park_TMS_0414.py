...
expName = 'park_TMS'
expInfo = {'Subject ID':'', 'Day':'', 'Run':'',
'TMS Site':["baseline","cTBS","iTBS","vertex"], 'Hand':["left", "right"]}
...
outfn = 'output_files/%s_Day%s_%s_%s_Run%s.csv' % (expInfo['Subject ID'], expInfo['Day'], expInfo['TMS Site'], expInfo['Hand'], expInfo['Run'])
reffn = 'output_files/%s_Day%s_%s_%s_Run1.csv' % (expInfo['Subject ID'], expInfo['Day'], expInfo['TMS Site'], expInfo['Hand'])
...
equalNum = 16 # number of scanner input before experiment starts
...
for ii in range(numTrials):
    trialList.append({'run': trialInfo['run'][ii],
                      ...
                      'pctFramesInTarget' : 0,
                      'arrFramesInTarget' : [],
                      'ypos_resp_vals': [], 'maxVal' : 0, 'minVal' : 0,
                      'resp': 0, 'correct': 0,'distFromTarget': [],
                      'sumSquared': 0,'RMSE': 0,
                      'avgDistFromTarget': 0
                      })
...
calibration_val_text = visual.TextStim(win,"force value: ", height = 0.1, pos = [-0.55,-0.8])
...
trialText = visual.TextStim(win=win, text="Experiment Trials", pos = (0,0), height=.2)
...
# ======================
if int(expInfo['Run']) == 1:  # calibrate on run 1
    ...
    while timer.getTime() > 0:
        force_vals = np.append(force_vals,joy.getY())
        calibration_val_text.text = "force value: %s" % str(round(joy.getY(), 3))
        countdownText.text = '%d' % int(np.ceil(timer.getTime()))
        countdownText.draw()
        calibration_text.draw()
        calibration_val_text.draw()
        win.flip()
    maxVal = np.max(force_vals[force_vals != 0.0])
    minVal = np.min(force_vals[force_vals != 0.0])

    # Let participant experiment with the crosshair
    timer.reset(5)
    calibration_text.text = "Sqeeze sensor to make the circle move"
    calibration_text.pos = [-0.55, -0.15]
    while timer.getTime() > 0:
        countdownText.text = '%d' % int(np.ceil(timer.getTime()))
...
else:  # if not run 1, read calibration columns from run 1 output file
    # read in run 1 output file
    run1Info = pd.read_csv(reffn)
    # select the calibration max and min information
    minVal = run1Info["minVal"][0]
    maxVal = run1Info["maxVal"][0]
...
for i in range(equalNum):
    startText.text = str(equalNum - i)
    startText.draw()
    inputText.draw()
    win.flip()
    while True:
...
for idx, trial in enumerate(trialList):
    ...
    # record the answer
    # divide the screen into four parts vertically, and answer will be determined
    # by which of the four parts the crosshair is in at the end of the response period
